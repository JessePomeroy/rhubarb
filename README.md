...
- Local SearXNG search, Crawl4AI Markdown extraction/site crawling, and Playwright MCP browsing
- Firecrawl preserved as an explicitly enabled paid fallback
- Asynchronous Pi, Claude Code, and Codex subagents with two-way parent/child messaging
- Optional separate interactive child panes in Herdr
...
- free-text, single-choice, and multi-select question popups
- shared skills for session analysis, PDF reading, YouTube transcripts, and browser debugging
...
## Review past Pi sessions

The shared `analyze-pi-sessions` skill is available through
`~/.pi/agent/skills/analyze-pi-sessions`. After `/reload`, ask naturally to find an
old discussion, review recurring corrections, or inspect a past session; explicitly
invoke `/skill:analyze-pi-sessions` when needed. For example: “Find our past Herdr
discussions” or “Review my recent corrections and suggest instruction improvements;
do not edit anything.”

Search and transcript helpers are local and read-only. `/usage` remains the live
session command. Historical usage reports are per session and account for Rhubarb
child-ledger revisions; they do not claim archive-wide spending totals.
