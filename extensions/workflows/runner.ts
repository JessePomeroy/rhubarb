...
function modelRuntimeFromRegistry(
  registry: ExtensionContext["modelRegistry"],
): ModelRuntime {
  const runtime = (registry as unknown as { runtime?: ModelRuntime }).runtime;
  if (!runtime) {
    throw new Error("The model registry does not expose its backing runtime");
  }
  return runtime;
}
  ...
  try {
    ...
    ({ session } = await createAgentSession({
      ...
      ...(options.thinkingLevel
        ...
        : {}),
      modelRuntime: modelRuntimeFromRegistry(options.modelRegistry),
