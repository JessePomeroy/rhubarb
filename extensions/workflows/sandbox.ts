...
export function runWorkflowSandbox(options: SandboxOptions) {
  return new Promise<unknown>((resolve, reject) => {
    ...
    child.on("exit", (code) => {
      if (!finished)
        finish(new Error(`Workflow sandbox exited (${code ?? "unknown"})`));
    });
    child.on("message", (raw) => {
      const message = raw as Record<string, unknown>;
      if (message.token !== token || typeof message.kind !== "string")
        return finish(new Error("Invalid workflow IPC message"));
      if (message.kind === "phase")
        return options.onPhase(String(message.title));
      if (message.kind === "meta") return options.onMeta(message.meta);
      if (message.kind === "result") return finish(undefined, message.result);
      if (message.kind === "error")
        return finish(new Error(String(message.error)));
      if (
        message.kind !== "agent" ||
        typeof message.id !== "number" ||
        typeof message.prompt !== "string"
      )
        return finish(new Error("Invalid workflow agent request"));
      if (++requests > 32)
        return finish(new Error("Workflow exceeded 32 agent calls"));
      const controller = new AbortController();
      active.set(message.id, controller);
      void options
        .onAgent(
          message.prompt,
          objectValue(message.options),
          controller.signal,
        )
        .then(
          (result) => {
